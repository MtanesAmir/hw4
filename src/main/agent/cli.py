...
import os
import sys
from .workflow import ResearchBugsAgentWorkflow

def main():
    parser = argparse.ArgumentParser(description="Run the autonomous ResearchBugsAgent workflow.")
    ...
    print(f"[CLI] Launching ResearchBugsAgentWorkflow...")
    print(f"Graph JSON: {input_abs}")
    print(f"Obsidian Vault: {vault_dir}")
    print(f"Project root: {project_root}")
    
    workflow = ResearchBugsAgentWorkflow(input_abs, vault_dir, project_root)
    workflow.run()
